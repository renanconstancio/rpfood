...
class ReportController extends Controller
{
    ...
    public function day_wise_report_export(Request $request){
        try{
            ...
            if ($request->type == 'excel') {
                return Excel::download(new VendorTransactionReportExport($data), 'Relatório de transação.xlsx');
            } else if ($request->type == 'csv') {
                return Excel::download(new VendorTransactionReportExport($data), 'Relatório de transação.csv');
            }
        }  catch(\Exception $e)
        {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
    ...
    public function order_report_export(Request $request)
    {
        try{
            ...
            if ($request->type == 'excel') {
                return Excel::download(new VendorOrderReportExport($data), 'Relatório do pedido.xlsx');
            } else if ($request->type == 'csv') {
                return Excel::download(new VendorOrderReportExport($data), 'Relatório do pedido.csv');
            }
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
    ...
    public function disbursement_report_export(Request $request,$type)
    {
        ...
        if($type == 'csv'){
            return Excel::download(new DisbursementVendorReportExport($data), 'Relatório de pagamento.csv');
        }
        return Excel::download(new DisbursementVendorReportExport($data), 'Relatório de pagamento.xlsx');
