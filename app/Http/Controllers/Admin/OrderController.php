...
class OrderController extends Controller
{
        ...
        try{
            ...
            $orders = Order::with(['customer', 'restaurant','refund'])
                ...
                if (in_array($status, ['requested','rejected','refunded']))
                {
                    ...
                    if ($type == 'excel') {
                        return Excel::download(new OrderRefundExport($data), 'Reembolsos.xlsx');
                    } else if ($type == 'csv') {
                        return Excel::download(new OrderRefundExport($data), 'Reembolsos.csv');
                    }
                }
            ...
            if ($type == 'excel') {
                return Excel::download(new OrderExport($data), 'Pedidos.xlsx');
            } else if ($type == 'csv') {
                return Excel::download(new OrderExport($data), 'Pedidos.csv');
            }
    ...
    public function orders_export(Request $request, $type, $restaurant_id){
        try{
            ...
            if($type == 'csv'){
                return Excel::download(new RestaurantOrderlistExport($data), 'Lista de pedidos.csv');
            }
            return Excel::download(new RestaurantOrderlistExport($data), 'Lista de pedidos.xlsx');
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
            return back();
