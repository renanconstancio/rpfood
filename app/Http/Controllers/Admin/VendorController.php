...
//        }
    ...
    public function restaurants_export( Request $request,  $type)
    {
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new RestaurantListExport($data), 'Restaurantes.csv');
            }
            return Excel::download(new RestaurantListExport($data), 'Restaurantes.xlsx');
    ...
    public function restaurant_wise_reviwe_export(Request $request){
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new RestauranrWiseFoodReviewExport($data), 'Restaurantes por avaliação de comidas.csv');
            }
            return Excel::download(new RestauranrWiseFoodReviewExport($data), 'Restaurantes por avaliação de comidas.xlsx');
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
            return back();
