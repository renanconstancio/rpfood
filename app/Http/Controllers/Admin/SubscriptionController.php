...
class SubscriptionController extends Controller
{
    ...
    public function package_list_export(Request $request)
    {
        ...
        $packages = SubscriptionPackage::withCount('transactions')
            ...
            if($request->type == 'csv'){
                return Excel::download(new SubscriptionPackageExport($data), 'Pacote de assinatura.csv');
            }
            return Excel::download(new SubscriptionPackageExport($data), 'Pacote de assinatura.xlsx');
        } catch(\Exception $e) {
    ...
    public function transcation_list_export(Request $request){
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new SubsPackageWiseTransactionExport($data), 'Transação de assinatura.csv');
            }
            return Excel::download(new SubsPackageWiseTransactionExport($data), 'Transação de assinatura.xlsx');
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
            return back();
