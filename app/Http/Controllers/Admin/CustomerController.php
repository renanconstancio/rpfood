...
class CustomerController extends Controller
{
    ...
    public function export(Request $request){
        try{
                ...
                if ($request->type == 'excel') {
                    return Excel::download(new CustomerListExport($data), 'Clientes.xlsx');
                } else if ($request->type == 'csv') {
                    return Excel::download(new CustomerListExport($data), 'Clientes.csv');
                }
            }  catch(\Exception $e){
    ...
    public function customer_order_export(Request $request)
    {
        try{
                ...
                if ($request->type == 'excel') {
                    return Excel::download(new CustomerOrderExport($data), 'ClientePedidos.xlsx');
                } else if ($request->type == 'csv') {
                    return Excel::download(new CustomerOrderExport($data), 'ClientePedidos.csv');
                }
            }  catch(\Exception $e){
                Toastr::error("line___{$e->getLine()}",$e->getMessage());
                info(["line___{$e->getLine()}",$e->getMessage()]);
    ...
    public function subscribed_customer_export(Request $request){
        try{
            ...
            if ($request->type == 'excel') {
                return Excel::download(new SubscriberListExport($data), 'Assinantes.xlsx');
            } else if ($request->type == 'csv') {
                return Excel::download(new SubscriberListExport($data), 'Assinantes.csv');
            }
        }  catch(\Exception $e){
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
