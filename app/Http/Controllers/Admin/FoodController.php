...
class FoodController extends Controller
{
    ...
    public function export(Request $request){
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new FoodListExport($data), 'Lista de alimentos.csv');
            }
            return Excel::download(new FoodListExport($data), 'Lista de alimentos.xlsx');
        }  catch(\Exception $e)
            {
    ...
    public function reviews_export(Request $request){
        try{
                ...
                if($request->type == 'csv'){
                    return Excel::download(new FoodReviewExport($data), 'Avaliações de alimentos.csv');
                }
                return Excel::download(new FoodReviewExport($data), 'Avaliações de alimentos.xlsx');
            }  catch(\Exception $e){
                Toastr::error("line___{$e->getLine()}",$e->getMessage());
                info(["line___{$e->getLine()}",$e->getMessage()]);
                return back();
    ...
    public function food_wise_reviews_export(Request $request){
        try{
                ...
                if($request->type == 'csv'){
                    return Excel::download(new FoodReviewExport($data), 'Comida por Avaliação.csv');
                }
                return Excel::download(new FoodReviewExport($data), 'Comida por Avaliação.xlsx');
            }  catch(\Exception $e){
                Toastr::error("line___{$e->getLine()}",$e->getMessage());
                info(["line___{$e->getLine()}",$e->getMessage()]);
                return back();
    ...
    public function restaurant_food_export($type, $restaurant_id){
        try{
            ...
            if($type == 'csv'){
                return Excel::download(new RestaurantFoodExport($data), 'Lista de alimentos.csv');
            }
            return Excel::download(new RestaurantFoodExport($data), 'Lista de alimentos.xlsx');
        }  catch(\Exception $e){
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
            return back();
