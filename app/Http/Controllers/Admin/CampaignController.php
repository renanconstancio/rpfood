...
class CampaignController extends Controller
{
    ...
    public function basic_campaign_export(Request $request){
        try{
            ...
            ->latest()->get();
            if($request->type == 'csv'){
                return Excel::download(new BasicCampaignExport($campaigns,$request['search']), 'Campanha.csv');
            }
            return Excel::download(new BasicCampaignExport($campaigns,$request['search']), 'Campanha.xlsx');
        }
            catch(\Exception $e)
        {
    ...
    public function item_campaign_export(Request $request){
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new FoodCampaignExport($campaigns,$request['search']), 'Campanha de comida.csv');
            }
            return Excel::download(new FoodCampaignExport($campaigns,$request['search']), 'Campanha de comida.xlsx');
            }
                catch(\Exception $e)
            {
                Toastr::error("line___{$e->getLine()}",$e->getMessage());
    ...
    public function food_campaign_list_export(Request $request){
        ...
        if($request->type == 'csv'){
            return Excel::download(new FoodCampaignOrderListExport($data), 'Lista de pedidos de campanha alimentar.csv');
        }
        return Excel::download(new FoodCampaignOrderListExport($data), 'Lista de pedidos de campanha alimentar.xlsx');
        }
                catch(\Exception $e)
            {
                Toastr::error("line___{$e->getLine()}",$e->getMessage());
